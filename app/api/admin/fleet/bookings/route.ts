...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all rentals for date range or default to +/- 6 months to prevent fetching entire database
    let allRentals;
    if (startDate && endDate) {
      allRentals = await kvRentalDB.getRentalsByDateRange(startDate, endDate);
    } else {
      // Default to 6 months back and 6 months forward if no dates specified
      ...
      console.log(`[BOOKINGS_API] No date range provided, defaulting to ${defaultStartDate} to ${defaultEndDate}`);
      allRentals = await kvRentalDB.getRentalsByDateRange(defaultStartDate, defaultEndDate);
    }
