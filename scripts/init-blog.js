...
// Run this script to populate the blog database with sample data
...
Ready to elevate your next corporate event? Contact our corporate services team to discuss your requirements and create a memorable experience for your clients.
    ...
    seo: {
      ...
      ogImage: '/images/blog/corporate-luxury.jpg',
      canonicalUrl: 'https://dtexoticslv.com/blog/corporate-events-luxury-transportation',
      noIndex: false,
      noFollow: false
    },
    status: 'published',
...
async function initializeBlogDatabase() {
  try {
    ...
    await kv.del('blog:posts:all');
    await kv.del('blog:categories:all');
    await kv.del('blog:tags:all');
    await kv.del('blog:posts:status:published');
    await kv.del('blog:posts:status:draft');
    await kv.del('blog:posts:status:archived');

    // Add categories
    console.log('📂 Adding categories...');
    for (const category of sampleCategories) {
      await kv.set(`blog:category:${category.id}`, category);
      await kv.sadd('blog:categories:all', category.id);
    }

    // Add tags
    console.log('🏷️  Adding tags...');
    for (const tag of sampleTags) {
      await kv.set(`blog:tag:${tag.id}`, tag);
      await kv.sadd('blog:tags:all', tag.id);
    }
    ...
    for (const post of samplePosts) {
      ...
      await kv.sadd('blog:posts:all', post.id);
      await kv.sadd(`blog:posts:status:${post.status}`, post.id);
      
      // Index by categories
      for (const category of post.categories) {
        await kv.sadd(`blog:posts:category:${category}`, post.id);
      }
      
      // Index by tags
      for (const tag of post.tags) {
        await kv.sadd(`blog:posts:tag:${tag}`, post.id);
      }
    }

    console.log('✅ Blog database initialized successfully!');
    console.log(`📊 Added ${samplePosts.length} posts, ${sampleCategories.length} categories, and ${sampleTags.length} tags`);
...
module.exports = { initializeBlogDatabase }; 